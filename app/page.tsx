...
export default function Home() {
  ...
  return (
    <main className="container mx-auto p-4 max-w-4xl min-h-screen flex flex-col justify-center">
      <Card className="mb-8">
        <CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {companies.map((company, index) => (
              <div key={index} className="space-y-4 border p-4 rounded-md">
                ...
                <div className="space-y-2">
                  <Label htmlFor={`company-background-${index}`}>
                    background
                  </Label>
                  <Textarea
                    id={`company-background-${index}`}
                    placeholder="E.g., Technology, Healthcare, Finance"
                    value={company.background}
                    rows={4}
                    onChange={(e) =>
                      handleCompanyChange(index, "background", e.target.value)
                    }
                    className="w-full"
                  />
                </div>
